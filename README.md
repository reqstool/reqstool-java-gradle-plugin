...
The `svcsAnnotationsFile` property (singular) from plugin `0.1.0` has been replaced by `svcsAnnotationsFiles` (plural, a file collection). The old property still works but emits a deprecation warning:

```groovy
// Deprecated — emits a warning and delegates to svcsAnnotationsFiles
requirementsTool {
